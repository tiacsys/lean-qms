...
this repo's own ``documents.yaml``, alongside this script — see that
file's own "Fields per group"/"Fields per document" docs for the
``dir:``/confdir convention). Each document's source directory is scanned
(top level only — a subdirectory such as ``appendix/`` holds templates with
their own, separate ``doc_control`` blocks, not this document's) for the one
``.rst`` file containing a ``.. doc_control::`` directive, whose fields
``author`` / ``reviewed_by`` / ``approved_by`` / ``approval_date`` /
``effective_date``
...
Versions" — the same ``<document-id>/*`` scheme ``conf_base.py``'s
``get_doc_version`` matches against to render the sidebar's version.
...
DEFAULT_REGISTRY = SCRIPT_DIR / "documents.yaml"
...
def _confdir_for(registry_path, document_id, data):
    ...
    if group_dir:
        return root / "docs" / group_dir / document_id
    return root / "docs" / document_id
...
def _update_directive_options(path, updates, directive_re):
    ...
    start = next((i for i, line in enumerate(lines) if directive_re.match(line.rstrip("\n"))), None)
    if start is None:
...
def _remove_doc_control_fields(path, keys):
    ...
    start = next((i for i, line in enumerate(lines) if DOC_CONTROL_RE.match(line.rstrip("\n"))), None)
    if start is None:
        raise DocctlError(f"No '.. doc_control::' directive found in {path}")
...
def cmd_approve(args):
    ...
    if args.effective_date:
        if not DATE_RE.match(args.effective_date):
            raise DocctlError(
                f"--effective-date must be YYYY-MM-DD, got: {args.effective_date!r}"
            )
...
def cmd_bump_version(args):
    ...
    _remove_doc_control_fields(
        doc_file, {"author", "reviewed_by", "approved_by", "approval_date"}
    )
...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        help=f"registry file to resolve document-id against (default: {DEFAULT_REGISTRY})",
    )
    ...
    p_review.add_argument(
        "--force",
        action="store_true",
        help="allow reviewing a document whose 'author' commit is your own "
        "(normally refused)",
    )
